...
read PHONE_NUMBER
# get customer_id
IFS="|" read CUSTOMER_ID NAME <<< $($PSQL "SELECT customer_id, name FROM customers WHERE phone='$PHONE_NUMBER'")
# if not found
if [[ -z $CUSTOMER_ID ]]
then
  ...
  # get name
  read NAME
  NAME_RESULT=$($PSQL "INSERT INTO customers (phone, name) VALUES ('$PHONE_NUMBER', '$NAME')")
